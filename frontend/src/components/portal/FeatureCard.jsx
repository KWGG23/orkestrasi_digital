...
export default function FeatureCard({ icon: Icon, eyebrow, title, description, to, badge, tone = 'primary', image }) {
  const toneClasses = tone === 'accent' ? 'bg-accent/10 text-accent' : 'bg-primary/10 text-primary'

  return (
    <Link
      to={to}
      className="group flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-white transition-shadow duration-200 hover:shadow-lg hover:shadow-primary/10"
    >
      {image && (
        <div className="h-36 w-full overflow-hidden bg-moss">
